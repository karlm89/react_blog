...
const AuthorShow = ({ match }) => {
	...
	useEffect(() => {
		axios.get(`https://jsonplaceholder.typicode.com/users/${match.params.authorID}`)
			.then((res) => {
				setUser({ 
		...
		axios.get(`https://jsonplaceholder.typicode.com/posts?userId=${match.params.authorID}`)
			.then((res) => {
				setPosts(res.data)
			})
			.catch((err) => {
	...
	return (
      ...
      <p className="ms-4">
        ...
        <span className = "ms-1">{user.address.zipcode}</span>
			...
			<p>
				website :{' '}
				<a href={`https://${user.data.website}`}>https://{user.data.website}</a>
			</p>
